...
int main() {
    ...
    // Utilização de OpenMP para paralelizar o cálculo das métricas
    #pragma omp parallel num_threads(4) // Especificar o uso de 5 threads
    {
        int thread_id = omp_get_thread_num() + 4;
        switch (thread_id) {
            case 4: // Thread 4 calcula WCC
                result_wcc = largest_wcc();
                break;
            case 5: // Thread 5 calcula SCC
                result_scc = largest_scc();
                break;
            case 6: // Thread 6 calcula o coeficiente de agrupamento
                result_clustering_coefficient = average_clustering_coefficient();
                result_triangles = calculate_triangles();
                break;
            case 7: // Thread 7 calcula diâmetro
                result_diameter = calculate_diameter();
                break;
        }
    }
    ...
    if (output_file.is_open()) {
        output_file << "======================\n";
        output_file << "Métricas de Uso do OPEN-MP\n";
        output_file << "======================\n";
